...
public class MissileBullet2 extends Bullet {
	...
	public MissileBullet2(AbstractWorldEngine world_engine, Uid uid, String name, Weapon2D owner, Vector2 location) {
		super(world_engine, uid, name, owner, location, new Vector2(600, 0), "vid:animation:objects/bullets", "player_missile_2", 1000f);
	...
	public void onTick(TickEvent tick_event) {
		...
		if (getLocation().distanceSquared(getVector2(Property.INITIAL_LOCATION)) > MAX_TRAVEL * MAX_TRAVEL) {
			...
			MissileBullet3 bullet1 = new MissileBullet3(world_engine, Uid.generateUid("vid:bullet", "MissileBullet3"), "MissileBullet3", (Weapon2D)getOwner(), getLocation().copy());
			bullet1.setLinearVelocity(new Vector2(600, 200));
			world_engine.insert(bullet1);
			region.queueEntityForAddition(bullet1);
			
			MissileBullet3 bullet2 = new MissileBullet3(world_engine, Uid.generateUid("vid:bullet", "MissileBullet3"), "MissileBullet3", (Weapon2D)getOwner(), getLocation().copy());
			bullet2.setLinearVelocity(new Vector2(600, 0));
			world_engine.insert(bullet2);
			region.queueEntityForAddition(bullet2);
			
			MissileBullet3 bullet3 = new MissileBullet3(world_engine, Uid.generateUid("vid:bullet", "MissileBullet3"), "MissileBullet3", (Weapon2D)getOwner(), getLocation().copy());
