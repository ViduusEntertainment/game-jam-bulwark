...
public class SpearheadWave extends EnemyWave {
	...
	protected void createEnemies(HashSet<NonPlayableCharacter2D> enemies) {
		for (int i = 0; i < 6; i++) {
			enemies.add(createStandardEnemy(new Vector2(world_engine.getWorldSize().width + (11 - i) * 30, 20 + i * 30)));
		}
		
		enemies.add(createStandardEnemy(new Vector2(world_engine.getWorldSize().width + 6 * 30, 20 + 6 * 30)));
